...
export default function TournamentOverview() {
  ...
  if (!data) return (
    <div className="auth-wrap">
      <div style={{ fontFamily: "var(--font-display)", fontSize: 13, fontWeight: 800, textTransform: "uppercase", letterSpacing: 2, color: "var(--muted)" }}>
        Loading…
      </div>
    </div>
  );
    ...
    <div style={{ minHeight: "100vh", background: "var(--bg)" }}>
      ...
      <div style={{ maxWidth: 1100, margin: "0 auto", padding: "28px 24px" }}>
        ...
        <div style={{ marginBottom: 28 }}>
          <h1 style={{ fontFamily: "var(--font-display)", fontSize: 30, fontWeight: 900, textTransform: "uppercase", letterSpacing: -1, color: "var(--ink)", margin: "0 0 8px" }}>
            {t.name}
          </h1>
          <div style={{ display: "flex", gap: 14, flexWrap: "wrap", fontSize: 13, color: "var(--muted)", alignItems: "center" }}>
            {t.venue && <span>📍 {t.venue}{t.city ? `, ${t.city}` : ""}</span>}
            {t.start_date && <span>📅 {t.start_date}</span>}
            <span className={`pill ${STATUS_PILL[t.status] || "pill-gray"}`} style={{ display: "inline-flex", alignItems: "center", gap: 5 }}>
        ...
        <div className="stats-grid" style={{ marginBottom: 20 }}>
          {[
            ...
            { label: "Live",    value: stats.live_matches, color: stats.live_matches > 0 ? "var(--primary)" : undefined },
        ...
        <div className="card share-link-card" style={{ display: "flex", alignItems: "center", gap: 12, marginBottom: 28 }}>
          ...
          <button className="btn btn-outline btn-sm" onClick={() => window.open(`/t/${t.slug}`, "_blank")}>
            View ↗
          </button>
        </div>
        ...
        {events.length === 0 ? (
          <div className="card" style={{ textAlign: "center", padding: "40px 20px" }}>
            ...
            <div style={{ fontFamily: "var(--font-display)", fontSize: 16, fontWeight: 900, textTransform: "uppercase", letterSpacing: -0.5, color: "var(--ink)", marginBottom: 6 }}>
              No Events Yet
            </div>
            <div style={{ fontSize: 13, color: "var(--muted)" }}>This tournament has no events configured.</div>
          </div>
        ) : (
          <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(280px,1fr))", gap: 12 }} className="events-grid">
            {events.map(ev => {
              ...
              return (
                <div
                  ...
                  style={{ borderTop: "3px solid var(--primary)", padding: "16px 18px" }}
                >
                  {/* Sport icon + name */}
                  <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 12 }}>
                    ...
                    <div>
                      <div style={{ fontFamily: "var(--font-display)", fontSize: 15, fontWeight: 900, textTransform: "uppercase", letterSpacing: -0.5, color: "var(--ink)" }}>
                        {ev.name}
                      </div>
                  ...
                  <div style={{ marginTop: 10, textAlign: "right", fontFamily: "var(--font-display)", fontSize: 11, fontWeight: 800, letterSpacing: 1, textTransform: "uppercase", color: "var(--primary)" }}>
                    Manage →
                  </div>
                </div>
              );
